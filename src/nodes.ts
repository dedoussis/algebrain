...
export class Node {
  ...
  equals(other: any): boolean {
    return typeof this === typeof other && this.value === other.value
  }
}
...
function evaluatePlus(children: List<Node>): Node {
  ...
  const sumNode: Num = nums.reduce((existing: Num, num: Num) => new Num(existing.value + num.value))
  if (nonNums.isEmpty()) {
    return sumNode
  }
  return new Operator(
...
function evaluateMul(children: List<Node>): Node {
  const nums: List<Num> = children.filter(child => child instanceof Num)
  const nonNums: List<Node> = children.filter(child => !(child instanceof Num))
  const mulNode: Num = nums.reduce((existing: Num, num: Num) => new Num(existing.value * num.value))
  if (nonNums.isEmpty()) {
    return mulNode
  }
  return new Operator(
